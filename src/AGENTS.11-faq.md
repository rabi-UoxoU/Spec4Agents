...
`.agents` 和 `.kiro` 是两个完全独立的目录，分别服务于不同的工具：

| 目录 | 使用者 | 配置文件 | 用途 |
|------|--------|---------|------|
| `.agents/` | 通用编码代理（Claude、Cursor 等） | `.config.agent` | 通用代理的 spec 工作空间 |
...
**事实**：两个目录路径完全不同（`.agents/` vs `.kiro/`），不存在文件覆盖的可能。
